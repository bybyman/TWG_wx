...
Page({
  ...
  onLoad: function () {
    ...
    self.imgViewHeight = self.device.windowHeight - 5 * self.deviceRatio
    self.setData({
  ...
  toCropPage(){
    ...
    wx.request({
        ...
        method: 'POST',
        data: {
          imgPath: wx.getFileSystemManager().readFileSync(this.data.tempImageSrc, "base64"),
        },
        success(res) {
          console.log(res.data)
          that.setData({
            genText: res.data.data.replace(/\n/g, '')
          })
